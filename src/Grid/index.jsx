...
export default function Grid() {
  ...
  const fetchData = async (x, y) => {
    try {
      const response = await fetch(`http://51.210.117.22:8080/monde/map?x_range=${x},${x}&y_range=${y},${y}`, {
        method: "GET",
        headers: {
      ...
      const extractedData = data.map(item => ({
        ...
        ressources: item?.terrain?.ressourcesPresente?.map(ress => ({
        ...
        {Array.from({ length: gridSize * gridSize }).map((_, index) => {
          ...
          return (
            <button
              ...
              className="w-16 h-16 bg-blue-500 text-white rounded-lg hover:bg-blue-700"
            >
              {x},{y}
            </button>
          );
      ...
      {apiResult.length > 0 && (
        <div className="border border-gray-300 rounded-lg p-4 shadow-md bg-white w-80">
          {apiResult.map((item, index) => (
            <div key={index}>
              <h2 className="text-xl font-bold text-gray-800 mb-2">{item.biomeNom}</h2>
                ...
                <ul className="space-y-2">
                  {item.ressources.map((ressource, resIndex) => (
                    <li key={resIndex} className="p-2 border rounded bg-gray-100">
                      <span className="font-semibold">{ressource.nom}</span> - {ressource.type}
                      <span className="ml-2 text-gray-600">(Quantité: {ressource.quantite})</span>
                    </li>
                  ))}
                </ul>
              ) : (
